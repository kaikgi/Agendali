...
import { Clock, User, Scissors, Phone, Mail, FileText, Save, CalendarClock } from 'lucide-react';
...
import { EstablishmentRescheduleDialog } from './EstablishmentRescheduleDialog';
...
const statusOptions: string[] = ['pending_approval', 'paid_pending_confirmation', 'pending_payment', 'booked', 'confirmed', 'arrived', 'in_service', 'completed', 'no_show', 'canceled', 'rejected'];

const rescheduleableStatuses = ['booked', 'confirmed', 'pending_approval', 'paid_confirmed', 'paid_pending_confirmation'];
...
export function AppointmentDetailsDialog({ open, onOpenChange, appointment }: AppointmentDetailsDialogProps) {
  ...
  const canReschedule = rescheduleableStatuses.includes(appointment.status) && !isPast;

  const handleStatusChange = async (newStatus: string) => {
    try {
      await updateStatus({ id: appointment.id, status: newStatus as any, oldStatus: appointment.status });
