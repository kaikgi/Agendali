...
serve(async (req: Request) => {
  ...
  try {
    const allowed = await checkRateLimit(req, "send-contact", 5, 60);
    if (!allowed) {
      return new Response(
        JSON.stringify({ error: "Muitas mensagens enviadas. Tente novamente mais tarde." }),
...
<table width="100%" style="max-width:560px;">
  ...
  <tr><td align="center" style="padding-bottom:24px;">
    <table cellpadding="0" cellspacing="0"><tr>
      <td style="background-color:#f0fdf4;border:1px solid #16a34a22;border-radius:100px;padding:8px 20px;">
        <span style="font-size:14px;font-weight:600;color:#16a34a;">✅ Mensagem Recebida</span>
      </td>
    </tr></table>
  </td></tr>
  <tr><td>
    <p style="margin:0 0 6px;font-size:16px;color:#374151;">Olá, <strong style="color:#111827;">${name}</strong>!</p>
  ...
  <tr><td style="padding-top:24px;border-top:1px solid #e5e7eb;">
    <p style="margin:0;text-align:center;font-size:12px;color:#9ca3af;line-height:1.6;">Este é um email automático. Por favor, não responda diretamente.</p>
