...
-- RPC: notify_appointment_status_change
-- Called from frontend after any status update. Handles all email job logic server-side.
...
BEGIN
  ...
  -- Fetch related data for payload
  ...
  CASE p_new_status
    ...
    WHEN 'confirmed' THEN
      -- Only send if transitioning from booked (manual confirmation)
      IF p_old_status = 'booked' THEN
