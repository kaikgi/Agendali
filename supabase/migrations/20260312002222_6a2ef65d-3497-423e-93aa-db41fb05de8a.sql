...
-- Update public_create_appointment to support tag-based approval bypass
...
BEGIN
  ...
  -- Determine status: check tag-based bypass for manual approval
  IF v_establishment.auto_confirm_bookings THEN
    v_status := 'confirmed';
  ELSE
    -- Check if customer has a tag with bypass_approval = true
  ...
  -- Create email jobs only for confirmed appointments
  IF v_clean_email IS NOT NULL AND v_status = 'confirmed' THEN
