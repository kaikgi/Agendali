...
BEGIN
  ...
  -- Check conflicts: include pending_approval and pending_payment in conflict check
  IF EXISTS (
  ...
  -- Only send emails for non-payment flows (payment flow emails are handled by webhook)
  IF NOT p_requires_payment AND v_clean_email IS NOT NULL AND length(v_clean_email) >= 4 THEN
